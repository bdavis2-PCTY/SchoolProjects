...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form.
	...
	if ($subject && $body) { // OK!
		...
		if (!$tid) { // Create a new thread.
			$q = "INSERT INTO CH17_threads (lang_id, user_id, subject) VALUES ({$_SESSION['lid']}, {$_SESSION['user_id']}, '" . mysqli_real_escape_string($dbc, $subject) . "')";
		...
		if ($tid) { // Add this to the replies table:
			$q = "INSERT INTO CH17_posts (thread_id, user_id, message, posted_on) VALUES ($tid, {$_SESSION['user_id']}, '" . mysqli_real_escape_string($dbc, $body) . "', UTC_TIMESTAMP())";
			$r = mysqli_query($dbc, $q);
			if (mysqli_affected_rows($dbc) == 1) {
